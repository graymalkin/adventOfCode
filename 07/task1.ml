...
let input_stream = ref []
let input () = pop input_stream

let output_stream = ref []
let output d =
  let phase = pop input_stream in
  push input_stream d;
  push input_stream phase;
  push output_stream d;
  printf ": %d\n" d
...
let amp mem value phase =
  input_stream := [phase; value; 0];
  run (Hashtbl.copy mem) 0;
  pop output_stream
...
let () =
  ...
  let perms = permutations [4;3;2;1;0] in
